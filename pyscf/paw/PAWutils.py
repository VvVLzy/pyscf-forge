...
def prepareMolForPAW(mol, auto_box=False):
    if auto_box:
        raise ValueError("Probably a units bug here but not using for now")
...
def getjSmoothPW2(cell, dm, mg_ni, mesh, PAWdata, Periodic=False):
    ...
    # Pass 1: AO -> Grid (Smooth Density)
    # rhoG from _eval_rhoG is shape (nset, nkpts, Ng)
    ...
    for i in range(nset):
        ...
        wv_freq = vG * dv
        # _get_j_pass2 expects (nset, nkpts, Ng). We wrap for single set.
...
def getjSmoothISDF(cell, dm, gaussgrid, aoOnR_tilde, mesh, PAWdata, Periodic=False):
    # RHF dm handling
    dm_half = dm[0, 0, :, :] / 2
    J = gaussgrid.compute_paw_coulomb(dm_half, PAWdata)
    return J*2
...
def getkSmoothISDF(cell, dm, gaussgrid, aoOnR_tilde, mesh, PAWdata, Periodic=False):
    # RHF dm handling
    dm_half = dm[0, 0, :, :] / 2
